...
class Intermediate:
    ...
    def backpropOptim(self,inState,opModelIndex,scorerIndex=(False,"Pass"),smuggle=None,adjAmountW=configs.adjAmountW,adjRangeW=configs.adjRangeW,adjAmountB=configs.adjAmountB,adjRangeB=configs.adjRangeB,adjRangeDef=None,boundsDef=None,adjAmountDef=None,stepsize=0.01,flip=True,iterationID=None,wBounds=configs.defaultBounds,bBounds=configs.defaultBounds,doEndpointScaling=False,batchCount=1,ascent=False,scoreNormalize=False):
    ...
    def sortOptim(self,inState,opModelIndex,scorerIndex=(False,"Pass"),smuggle=None,adjAmountW=configs.adjAmountW,adjRangeW=configs.adjRangeW,adjAmountB=configs.adjAmountB,adjRangeB=configs.adjRangeB,adjRangeDef=None,boundsDef=None,adjAmountDef=None,flip=True,iterationID=None,wBounds=configs.defaultBounds,bBounds=configs.defaultBounds,doEndpointScaling=False,batchCount=1,ascent=False,scoreNormalize=False):
        ...
        for each in adjAmountList:
            ...
            resultsList.append((modScore,each))

        resultsList.sort()
        if ascent:
            self.mdlDict[opModelIndex].adjustElement(resultsList[len(resultsList)-1][1])
            algs.printToShallow(f"{resultsList[len(resultsList)-1][0]}\n")
        else:
            self.mdlDict[opModelIndex].adjustElement(resultsList[0][1])
            algs.printToShallow(f"{resultsList[0][0]}\n")
        
        self.mdlDict[opModelIndex].purgeLAE()
        return self.mdlDict[opModelIndex].runModel(inState)[1]
